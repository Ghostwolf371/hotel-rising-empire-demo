...
- **Management `/management/*`** (except the login page) uses DB staff accounts (`ManagementUser`) when present; sign-in via `app/actions/management-auth.ts`, user CRUD at `/management/users`. Env `MANAGEMENT_EMAIL` + `MANAGEMENT_PASSWORD` bootstraps the first admin on seed and remains a fallback login only while no DB users exist. Session is an httpOnly cookie verified in `proxy.ts` (HMAC) and server actions (active user check).
